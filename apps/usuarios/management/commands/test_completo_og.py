...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
      ...
      if Pregunta.objects.filter(test=test).exists():
          self.stdout.write(self.style.WARNING('Las preguntas para este test ya existen.'))
          return
      preguntas = [
        # ── ÁREA: TIEMPO ──────────────────────────────────────────
          {
      ...
      self.stdout.write(self.style.SUCCESS( f'✅ {Pregunta.objects.filter(test=test).count()} preguntas creadas en el test.'))